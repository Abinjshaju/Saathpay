...
export default function MembersPage() {
  ...
  // Build plan-based filters dynamically
  const planFilters = useMemo(() => {
    const planNames = new Set(members.map((m) => m.plan_name).filter(Boolean));
    return [
      ...filters,
      ...Array.from(planNames).map((name) => ({ label: name!, value: name! })),
    ];
  }, [members]);

  const filtered = useMemo(() => {
    let list = members;
    if (filter !== "all") list = list.filter((m) => m.plan_name === filter);
    if (search.trim()) {
      const q = search.toLowerCase();
      list = list.filter((m) => m.full_name.toLowerCase().includes(q) || (m.mobile && m.mobile.includes(q)));
    }
    return list;
          ...
          <table className="w-full border-collapse text-left">
            <thead>
              <tr className="border-b border-border">
                ...
                <th className="px-5 py-3 text-xs font-bold uppercase tracking-wider text-ink-muted">Mobile</th>
                <th className="px-5 py-3 text-xs font-bold uppercase tracking-wider text-ink-muted">Plan</th>
                <th className="px-5 py-3 text-xs font-bold uppercase tracking-wider text-ink-muted">Amount</th>
              ...
              {filtered.map((m) => (
                <tr key={m.id} className="transition-colors hover:bg-bg">
                  <td className="px-5 py-4">
                    <Link to={`/members/${m.id}`} className="text-sm font-semibold text-ink hover:underline">{m.full_name}</Link>
                  </td>
                  <td className="px-5 py-4 text-sm text-ink-secondary">{m.mobile ?? "—"}</td>
                  <td className="px-5 py-4 text-sm text-ink-secondary">{m.plan_name ?? "—"}</td>
