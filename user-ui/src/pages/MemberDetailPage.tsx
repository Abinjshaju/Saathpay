...
export default function MemberDetailPage() {
  ...
  const { payments, loading: paymentsLoading, addPayments, updatePayment } = usePayments(id);
  ...
  const feePerMonth = Number(member.plan_amount ?? 0);
  ...
  async function handleRecordPayment() {
    ...
    const rows: { member_id: string; month: string; date: string; amount: number; status: string }[] = [];
  ...
  function openRecordForm() {
    setShowRecord(true);
    setSelectedMonths(new Set());
    setRecordAmount(String(feePerMonth));
    setPickerYear(new Date().getFullYear());
  }

  return (
    <div className="flex flex-1 flex-col bg-bg">
      <PageHeader title={member.full_name} showBack />
      ...
      <div className="space-y-4 px-5 pt-4 lg:mx-auto lg:w-full lg:max-w-2xl lg:space-y-6 lg:px-6 lg:py-6">
        ...
        <div className="list-card px-5 py-5">
          <div className="grid grid-cols-2 gap-4">
            <div>
              <p className="text-xs font-bold uppercase tracking-wider text-ink-muted">Mobile</p>
              <p className="mt-1 text-sm font-medium text-ink">{member.mobile ?? "—"}</p>
            </div>
            <div>
              <p className="text-xs font-bold uppercase tracking-wider text-ink-muted">Plan</p>
              <p className="mt-1 text-sm font-bold text-ink">{member.plan_name ?? "—"}</p>
              {member.plan_amount != null && (
                <p className="text-xs tabular-nums text-ink-secondary">₹{Number(member.plan_amount).toLocaleString("en-IN")}</p>
              )}
            </div>
            {member.email && (
              <div>
                <p className="text-xs font-bold uppercase tracking-wider text-ink-muted">Email</p>
                <p className="mt-1 text-sm text-ink">{member.email}</p>
              </div>
            )}
            {member.next_due_date && (
              <div>
                <p className="text-xs font-bold uppercase tracking-wider text-ink-muted">Next Due</p>
                <p className="mt-1 text-sm text-ink">{member.next_due_date}</p>
              </div>
            )}
          </div>
      ...
      <ConfirmDialog
        open={showDelete}
        title="Delete Member"
        message={`Are you sure you want to delete ${member.full_name}? This action cannot be undone and all payment history will be lost.`}
        confirmLabel="Delete"
        cancelLabel="Cancel"
        destructive
